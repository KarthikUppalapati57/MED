...
export default function LandingPage() {
  ...
  const navBackground = useTransform(smoothProgress, [0, 0.05], ["rgba(5, 5, 8, 0)", "rgba(5, 5, 8, 0.85)"]);
  const navBorder = useTransform(smoothProgress, [0, 0.05], ["rgba(255, 255, 255, 0)", "rgba(255, 255, 255, 0.05)"]);
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-white selection:bg-[#ff5c35]/30 font-sans antialiased overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8 text-[11px] font-bold tracking-[2px] text-white/60 uppercase">
              ...
              <a href="#showcase" className="hover:text-white transition-colors">Telemetry</a>
              <a href="#pricing" className="hover:text-white transition-colors">Pricing</a>
              <div className="h-4 w-[1px] bg-white/10 mx-2" />
              <button className="hover:text-white transition-colors" onClick={() => navigate('/login')}>Log in</button>
              
              <MagneticButton 
                className="bg-white text-black hover:bg-[#ff5c35] hover:text-white font-bold text-[10px] tracking-[2px] h-9 px-5 rounded-full transition-all uppercase"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          ...
          >
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#ff5c35]/10 border border-[#ff5c35]/20 text-[10px] font-bold tracking-[3px] uppercase text-[#ff5c35] mb-8"
            >
              <Sparkles className="h-3 w-3" />
              Intelligence Orchestration
            </motion.div>
            
            <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-[110px] leading-[0.9] font-bold tracking-tighter text-white mb-8">
              <motion.span 
            ...
            >
              <MagneticButton 
                className="h-14 px-10 bg-white text-black hover:bg-[#ff5c35] hover:text-white font-bold text-xs tracking-[3px] rounded-full transition-all uppercase shadow-[0_0_40px_-10px_rgba(255,255,255,0.3)] hover:shadow-[0_0_40px_-10px_rgba(255,92,53,0.6)]" 
                onClick={() => setIsDemoModalOpen(true)}
              >
                REQUEST ACCESS <ChevronRight className="ml-2 h-4 w-4" />
              </MagneticButton>
              <Link to="/docs" className="text-white/60 font-bold text-xs tracking-[3px] uppercase hover:text-white transition-colors flex items-center group">
                VIEW DOCUMENTATION 
                <span className="block ml-2 w-0 h-[1px] bg-white group-hover:w-4 transition-all duration-300" />
              </Link>
            </motion.div>
          </motion.div>
        </div>
      ...
      <div className="relative z-10 bg-transparent border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col lg:flex-row items-center gap-32">
              ...
              >
                ...
                <h2 className="text-5xl md:text-7xl font-bold tracking-tighter leading-[1] text-white">
                  Decentralized <br /> <span className="text-white/40">Kitchen Ops.</span>
                </h2>
        ...
        <section id="features" className="py-32 bg-black/20 backdrop-blur-sm border-y border-white/5">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <h2 className="text-4xl md:text-6xl font-bold tracking-tighter mb-6 text-white">Infrastructure Modules</h2>
              <p className="text-white/40 max-w-xl text-lg">Completely modular, enterprise-grade systems designed to automate the manual toil of restaurant management.</p>
            </motion.div>
            ...
            >
              <div className="relative group p-[1px] rounded-3xl bg-gradient-to-b from-white/20 to-white/5 hover:from-[#ff5c35]/50 transition-all duration-700">
                ...
                <div className="relative p-12 bg-[#0a0a0f]/60 rounded-3xl h-full backdrop-blur-xl">
                  ...
                  <div className="mb-12">
                    <h3 className="text-2xl font-bold tracking-tight mb-6 text-white">Platform Complete</h3>
                    <div className="flex items-baseline gap-2">
                      <span className="text-7xl font-bold tracking-tighter text-white">$149</span>
                      <span className="text-white/30 font-bold text-sm uppercase tracking-[2px]">/ Mo</span>
                    </div>
                  </div>
                  <ul className="space-y-6 mb-12">
                    {["Unlimited visual extractions", "Universal user access", "Full-stack telemetry", "Dedicated API instance", "24/7 technical escort"].map((item, i) => (
                      <li key={i} className="flex items-center gap-4 text-white/60 text-xs font-medium uppercase tracking-widest">
                        <CheckCircle2 className="w-4 h-4 text-[#ff5c35]" />
                        {item}
                      </li>
                    ))}
                  </ul>
                  <MagneticButton 
                    className="w-full h-14 bg-white text-black hover:bg-[#ff5c35] hover:text-white font-bold text-[10px] tracking-[4px] rounded-full uppercase transition-all shadow-xl" 
                    onClick={() => setIsDemoModalOpen(true)}
                  >
                    JOIN WAITLIST
                  </MagneticButton>
      ...
      <Dialog open={isDemoModalOpen} onOpenChange={setIsDemoModalOpen}>
        <DialogContent className="sm:max-w-[450px] bg-[#0a0a0f] border-white/10 text-white p-0 overflow-hidden rounded-2xl">
          ...
          <div className="p-10">
            <DialogHeader className="mb-10">
              ...
              <DialogDescription className="text-white/40 text-xs font-medium tracking-widest uppercase mt-3">
                Initialize your workspace.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleDemoSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="fullName" className="text-[10px] font-bold uppercase tracking-[2px] text-white/40">Full Identity</Label>
                <Input id="fullName" required value={demoForm.fullName} onChange={(e) => setDemoForm({...demoForm, fullName: e.target.value})} placeholder="John Doe" className="bg-white/5 border-white/10 h-12 text-white placeholder:text-white/20 focus:ring-[#ff5c35] rounded-xl text-sm transition-all" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="email" className="text-[10px] font-bold uppercase tracking-[2px] text-white/40">Technical Email</Label>
                <Input id="email" type="email" required value={demoForm.email} onChange={(e) => setDemoForm({...demoForm, email: e.target.value})} placeholder="john@ops.com" className="bg-white/5 border-white/10 h-12 text-white placeholder:text-white/20 focus:ring-[#ff5c35] rounded-xl text-sm transition-all" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="companyName" className="text-[10px] font-bold uppercase tracking-[2px] text-white/40">Enterprise Name</Label>
                <Input id="companyName" required value={demoForm.companyName} onChange={(e) => setDemoForm({...demoForm, companyName: e.target.value})} placeholder="Acme Logistics" className="bg-white/5 border-white/10 h-12 text-white placeholder:text-white/20 focus:ring-[#ff5c35] rounded-xl text-sm transition-all" />
              </div>
              <DialogFooter className="pt-6">
                <Button type="submit" disabled={isSubmitting} className="w-full h-12 bg-white text-black hover:bg-[#ff5c35] hover:text-white font-bold text-xs tracking-[3px] rounded-xl uppercase transition-all">
...
const StatItem = ({ value, label }) => (
  ...
  >
    <div className="text-4xl md:text-5xl font-bold tracking-tighter mb-3 text-white">{value}</div>
    <div className="text-[9px] text-white/40 font-bold uppercase tracking-[3px]">{label}</div>
  </motion.div>
);
...
const CheckItem = ({ title, desc }) => (
  <div className="space-y-1 group">
    <div className="flex items-center gap-3">
      <CheckCircle2 className="h-4 w-4 text-[#ff5c35] opacity-80" />
      <h4 className="font-bold text-xs tracking-[1px] uppercase text-white/90">{title}</h4>
    </div>
    <p className="text-[10px] text-white/40 font-medium uppercase tracking-widest pl-7">{desc}</p>
  </div>
);
  ...
  >
    <div className="relative p-8 h-full bg-[#0a0a0f] rounded-3xl overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="mb-8 text-white/60 group-hover:text-[#ff5c35] transition-colors duration-500">
          {React.cloneElement(icon, { size: 28, strokeWidth: 1.5 })}
        </div>
        <h3 className="text-lg font-bold tracking-tight mb-3 text-white">{title}</h3>
