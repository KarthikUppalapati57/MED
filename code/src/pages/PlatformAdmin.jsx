...
export default function PlatformAdmin() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    enabled:
      authChecked && (userRole === "admin" || userRole === "platform_admin"),
  ...
  const updateRequest = useMutation({
    mutationFn: async ({ id, approved }) => {
      ...
      if (approved) {
        import("sonner").then(({ toast }) =>
          toast.info(
            "Request approved! In MEVS, user accounts must be linked manually via invitations instead of auto-provisioning for security.",
          ),
        );
      }
      return true;
  ...
  const revokeRequest = useMutation({
    ...
    onSettled: (data, error, { id }) => {
      ...
      if (error) {
        ...
        import("sonner").then(({ toast }) =>
          toast.success("Access revoked. User must be re-approved."),
        );
      }
      queryClient.invalidateQueries({ queryKey: ["access-requests"] });
    },
  ...
  const handleBulkDelete = () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${selectedRequests.size} requests?`,
      )
    ) {
  ...
  const pendingAccessCount = accessReqs.filter(
    (r) => r.status === "pending_approval",
  ).length;
  const pendingContactCount = contactReqs.filter(
    (r) => r.status === "pending_approval",
  ).length;
  const pendingOrgCount = orgs.filter((o) =>
    ["under_review", "pending_approval", "onboarding"].includes(o.status),
  ).length;
  const pendingCount =
    pendingAccessCount + pendingContactCount + pendingOrgCount;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center px-4">
        ...
        <p className="text-slate-500 max-w-md">
          The Platform Admin Console is restricted to platform administrators
          only. If you believe this is an error, contact your system
          administrator.
        </p>
      </div>
    );
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <div className="relative">
            ...
            <Input placeholder="Search..." className="pl-9 w-48 h-8" />
          </div>
            ...
            <TableRow className="bg-slate-50">
              <TableHead className="w-10">
                <Checkbox
                  checked={
                    list.length > 0 &&
                    list.every((r) => selectedRequests.has(r.id))
                  }
            ...
            {isLoading ? (
              ...
              list.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="text-sm">
                    {r.portfolios || r.properties_count || "—"}
                  </TableCell>
                  <TableCell className="text-sm">
                    {r.plan ? (
                      <Badge
                  ...
                  <TableCell>
                    {r.request_type === "demo" ? (
                      <Badge className="bg-violet-100 text-violet-700 text-[10px] border-none font-bold">
                        🎥 DEMO
                      </Badge>
                    ) : r.request_type === "contact" ? (
                      <Badge className="bg-indigo-100 text-indigo-700 text-[10px] border-none font-bold">
                        ✉️ CONTACT
                      </Badge>
                    ) : (
                      <Badge className="bg-emerald-100 text-emerald-700 text-[10px] border-none font-bold italic">
                        ⚡ ACCESS
                      </Badge>
                    )}
                  </TableCell>
                            ...
                            <div className="space-y-4 py-2">
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                {r.request_type === "demo" && (
                                  <div>
                                    ...
                                    <p
                                      className={`font-semibold text-sm ${r.demo_viewed ? "text-emerald-600" : "text-slate-500"}`}
                                    >
                              ...
                              {r.message && (
                                <div>
                                  ...
                                  <p className="bg-slate-50 p-3 rounded-md text-sm border border-slate-100 whitespace-pre-wrap">
                                    {r.message}
                                  </p>
                                </div>
                              )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Platform-wide management · CRE Admin Version · MEVS Deployment
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={showInviteModal} onOpenChange={setShowInviteModal}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Generate a secure platform invitation token for a new tenant
                  administrator.
                </DialogDescription>
              </DialogHeader>
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-[10px] text-blue-500">Live deployments</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-[10px] text-violet-500">
              {demoRequests.filter((r) => r.demo_viewed).length} viewed
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-[10px] text-emerald-500">+8.2% MoM</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-[10px] text-slate-400">
              {pendingOrgCount > 0
                ? `${pendingOrgCount} org${pendingOrgCount > 1 ? "s" : ""} awaiting activation`
                : "Access and contact requests"}
            </p>
          </CardContent>
        </Card>
      </div>
                      ...
                      <TableRow key={r.id}>
                        ...
                        <TableCell className="text-sm">
                          {r.plan ? (
                            <Badge
                              variant="outline"
                              className="text-[10px] capitalize bg-violet-50 text-violet-700 border-violet-100"
                            >
                              {r.plan}
                            </Badge>
                          ) : (
                        ...
                        <TableCell>
                          <Badge
                            className={
                              r.demo_viewed
                                ? "bg-emerald-100 text-emerald-700 border-none text-[10px]"
                                : "bg-slate-100 text-slate-500 border-none text-[10px]"
                            }
                          >
                            {r.demo_viewed ? "✓ Watched" : "Not yet"}
                          </Badge>
                          ...
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                if (
                                  !window.confirm(
                                    "Delete this demo request permanently?",
                                  )
                                )
                                  return;
                                const { error } = await supabase
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Manage users with full administrative access to the platform.
                </p>
              </div>
              <Dialog
                open={showPlatformInviteModal}
                onOpenChange={setShowPlatformInviteModal}
              >
                <DialogTrigger asChild>
                  <Button
                    size="sm"
                ...
                <DialogContent>
                  ...
                  <DialogFooter>
                    <Button
                      variant="ghost"
                      onClick={() => setShowPlatformInviteModal(false)}
                    >
                      Close
                    </Button>
                    <Button
                      onClick={handleInvitePlatformAdmin}
                      disabled={platformInviting || !platformInviteEmail}
                    >
                      Generate Token
                    </Button>
                  </DialogFooter>
                  ...
                  {isLoadingAdmins ? (
                    ...
                    platformAdmins.map((admin) => (
                      <TableRow key={admin.id}>
                        ...
                        <TableCell>
                          <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100 border-none text-[10px]">
                            Platform Admin
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <Button
                            ...
                            onClick={async () => {
                              if (
                                !window.confirm(
                                  `Demote ${admin.email} from platform admin?`,
                                )
                              )
                                return;
                              try {
        ...
        <TabsContent value="orgs" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <p className="text-xs text-slate-400">
                Configure which modules each organization can access. Empty =
                all modules (admin/legacy).
              </p>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                  ...
                  {orgs.length === 0 ? (
                    ...
                    [...orgs]
                      .sort((a, b) => {
                        ...
                        const aIsPending = pendingStatuses.includes(
                          a.status || a.subscription_status,
                        );
                        const bIsPending = pendingStatuses.includes(
                      ...
                      .map((org) => {
                        const isPending = [
                          ...
                          "pending_approval",
                          "onboarding",
                        ].includes(org.subscription_status || org.status);
                            ...
                            <TableCell>
                              <div className="font-medium text-sm flex items-center gap-2">
                                ...
                                {isPending && (
                                  <Badge
                                    variant="outline"
                                    className="text-[9px] h-4 bg-amber-100 text-amber-700 border-amber-200 uppercase"
                                  >
                                    Attention
                                  </Badge>
                                )}
