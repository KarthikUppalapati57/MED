...
```ts
  ...
  13 |     // or show an unauthorized message.
  14 |     const isLogin = await page.locator('text=Sign in to EdgeOps').count() > 0;
  15 |     const isUnauthorized = await page.locator('text=Unauthorized').count() > 0;
  16 |     
> 17 |     expect(isLogin || isUnauthorized).toBeTruthy();
     |                                       ^ Error: expect(received).toBeTruthy()
  18 |   });
  19 | });
  20 | 
