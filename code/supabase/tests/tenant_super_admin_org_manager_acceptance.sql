...
DECLARE
  ...
  v_brand_manager uuid := gen_random_uuid();
...
BEGIN
  ...
  VALUES
    ...
    (v_brand_manager, v_tenant_a, v_org_a, v_brand_a, NULL, 'brand-manager@example.test', 'Brand Manager', 'brand_manager', 'brand', 'active'),
  ...
  VALUES
    ...
    (v_org_a, v_brand_manager, 'brand_manager'),
  ...
  VALUES
    ('role_rank_order', public.access_role_rank('location_manager') < public.access_role_rank('brand_manager') AND public.access_role_rank('brand_manager') < public.access_role_rank('org_manager') AND public.access_role_rank('org_manager') < public.access_role_rank('tenant_super_admin') AND public.access_role_rank('tenant_super_admin') < public.access_role_rank('platform_admin'), 'role rank order'),
    ...
    ('tenant_ids_present_for_scoped_roles', NOT EXISTS (
       ...
       WHERE id IN (v_tenant_admin, v_org_manager, v_brand_manager, v_location_manager)
         AND tenant_id IS NULL
     ), 'tenant_id exists for tenant/org/brand/location scoped users'),
    ('brand_manager_role_supported', public.access_role_rank('brand_manager') > public.access_role_rank('location_manager'), 'brand manager rank and role support'),
