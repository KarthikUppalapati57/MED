...
-- submit_vendor_banking_via_link (the RPC of record for the public banking-link submission path)
-- only ever checked token validity -- routing/account FORMAT was validated only by the
-- best-effort, non-fatal Dwolla helper downstream, so a malformed value still persists to
-- vendor_banking_details/vault whenever the Dwolla call is skipped or fails. Add the same
-- digit-count check store_bank_account_secret already applies on the provider-neutral path, so
-- both paths agree on what a valid bank account looks like.
