-- Reset incomplete legacy trial-coupon bypasses so users collect a Stripe payment method.
...
SET payment_verified = false,
    ...
    pending_payment_metadata = COALESCE(pending_payment_metadata, '{}'::jsonb) || jsonb_build_object(
      'provider', 'stripe',
      'previous_provider', COALESCE(payment_method_type, 'unknown'),
      'requires_payment_method_collection', true,
      'reset_reason', 'trial_coupon_requires_stripe_payment_method',
      'reset_at', now()
    ),
    updated_at = now()
WHERE organization_id IS NULL
  AND COALESCE(payment_method_type, '') = 'trial_coupon'
  AND pending_checkout_session_id IS NULL;
-- Require Stripe payment-method collection for trial coupons before hierarchy setup.
