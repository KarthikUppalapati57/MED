-- 075: Role dashboard summary RPC
-- Aggregates role-scoped operator dashboard metrics for the Restops Home/Dashboard.
...
BEGIN
  ...
  scoped_sales AS (
    ...
    WHERE ps.organization_id = p_org_id
      AND ps.date BETWEEN (v_last_year_week_start - interval '7 days')::date AND p_period_end
      AND (
        p_scope = 'org'
        OR ps.location_id IN (SELECT id FROM scoped_locations)
        OR ps.location_id IS NULL
      )
  ),
  scoped_invoices AS (
    SELECT i.*
  ...
  scoped_inventory AS (
    ...
    WHERE inv.organization_id = p_org_id
      AND (
        p_scope = 'org'
        OR (p_scope = 'brand' AND (inv.brand_id = p_brand_id OR (inv.brand_id IS NULL AND inv.location_id IN (SELECT id FROM scoped_locations))))
        OR (p_scope IN ('location', 'staff') AND (inv.location_id = p_location_id OR inv.location_id IS NULL))
      )
  ),
  scoped_products AS (
    SELECT p.*
  ...
  kpis AS (
    SELECT
      COALESCE(SUM(ps.revenue) FILTER (WHERE ps.date = v_today), 0) AS sales_today,
      COALESCE(SUM(ps.revenue) FILTER (WHERE ps.date BETWEEN v_week_start AND (v_week_start + 6)), 0) AS sales_week_to_date,
      COALESCE(SUM(ps.revenue) FILTER (WHERE ps.date BETWEEN v_last_week_start AND (v_last_week_start + 6)), 0) AS sales_last_week,
      COALESCE(SUM(ps.revenue) FILTER (WHERE ps.date BETWEEN v_last_year_week_start AND (v_last_year_week_start + 6)), 0) AS sales_last_year,
      COALESCE(SUM(ps.revenue) FILTER (WHERE ps.date BETWEEN p_period_start AND p_period_end), 0) AS sales_period
    FROM scoped_sales ps
  ),
  costs AS (
    SELECT
      COALESCE((SELECT SUM(total_amount) FROM period_invoices), 0) AS invoice_spend,
      ...
      COALESCE((SELECT SUM(total_amount) FROM scoped_invoices WHERE payment_status = 'unpaid' OR status = 'approved'), 0) AS unpaid_amount,
      COALESCE((SELECT COUNT(*) FROM scoped_invoices WHERE status = 'pending_review'), 0) AS pending_invoices,
      COALESCE((SELECT COUNT(*) FROM scoped_inventory WHERE COALESCE(current_quantity, 0) <= COALESCE(reorder_point, 5)), 0) AS low_stock_items,
      ...
      COALESCE((SELECT COUNT(*) FROM scoped_invoices), 0) AS total_invoices,
      ...
      COALESCE((SELECT COUNT(*) FROM scoped_inventory), 0) AS total_inventory_items
  ),
  sales_rows AS (
    SELECT jsonb_agg(
      jsonb_build_object(
    ...
    FROM (
      SELECT
        d::date AS day_date,
        COALESCE((SELECT SUM(revenue) FROM scoped_sales WHERE date = d::date), 0) AS actual,
        COALESCE((SELECT SUM(revenue) FROM scoped_sales WHERE date = (d::date - interval '7 days')::date), 0) AS last_week,
        COALESCE((SELECT SUM(revenue) FROM scoped_sales WHERE date = (d::date - interval '1 year')::date), 0) AS last_year
